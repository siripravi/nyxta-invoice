...
(function (window, angular) {
  ...
  describe("$cookies", function () {
    ...
    beforeEach(function () {
      mockedCookies = {};
      module("ngCookies", {
        $$cookieWriter: jasmine
          .createSpy("$$cookieWriter")
          .and.callFake(function (name, value) {
    ...
    it("should deserialize json to object", inject(function ($cookies) {
      ...
      expect($cookies.getObject("objectCookie")).toEqual({
        id: 123,
        name: "blah",
      });
    }));

    it("should delete objects from the store when remove is called", inject(function (
      $cookies
    ) {
    ...
    it("should put cookie value without serializing", inject(function (
      $cookies
    ) {
    ...
    it("should get cookie value without deserializing", inject(function (
      $cookies
    ) {
      $cookies.put("name", "value");
    ...
    it("should pass options on put", inject(function (
      $cookies,
      $$cookieWriter
    ) {
      $cookies.put("name", "value", { path: "/a/b" });
      expect($$cookieWriter).toHaveBeenCalledWith("name", "value", {
        path: "/a/b",
      });
    }));

    it("should pass options on putObject", inject(function (
      $cookies,
      $$cookieWriter
    ) {
    ...
    it("should pass options on remove", inject(function (
      $cookies,
      $$cookieWriter
    ) {
      $cookies.remove("name", { path: "/a/b" });
      expect($$cookieWriter).toHaveBeenCalledWith("name", undefined, {
        path: "/a/b",
      });
    }));
  ...
  describe("$$cookieWriter", function () {
    ...
    describe("put via $$cookieWriter(cookieName, string)", function () {
      ...
      it("should log warnings when 4kb per cookie storage limit is reached", inject(function (
        $log
      ) {
        var i,
          longVal = "",
        ...
        if (document.cookie !== cookieStr) {
          this.fail(
            new Error(
              "browser didn't drop long cookie when it was expected. make the " +
  ...
  describe("cookie options", function () {
    ...
    it("should accept expires option on set", function () {
      $$cookieWriter("name", "value", {
        expires: "Fri, 19 Dec 2014 00:00:00 GMT",
      });
      expect(getLastCookieAssignment("expires")).toMatch(
    ...
    it("should accept date object as expires option", function () {
      $$cookieWriter("name", "value", {
        expires: new Date(Date.UTC(1981, 11, 27)),
      });
      expect(getLastCookieAssignment("expires")).toMatch(
