...
(function (window, angular) {
  ...
  InterpolationParts.prototype.getExpressionValues =
    function getExpressionValues(context) {
  ...
  InterpolationParts.prototype.getResult = function getResult(
    expressionValues
  ) {
  ...
  MessageFormatParser.prototype.errorInParseLogic =
    function errorInParseLogic() {
  ...
  MessageFormatParser.prototype.assertRuleOrNull = function assertRuleOrNull(
    rule
  ) {
    if (rule === undefined) {
      this.errorInParseLogic();
  ...
  MessageFormatParser.prototype.startStringAtMatch =
    function startStringAtMatch(match) {
  ...
  MessageFormatParser.prototype.ruleSelectKeyword =
    function ruleSelectKeyword() {
  ...
  MessageFormatParser.prototype.ruleInInterpolationOrMessageText =
    function ruleInInterpolationOrMessageText() {
      ...
      if (token === "{{") {
        ...
        this.choices[this.choiceKey] = this.interpolationParts.toParsedFn(
          /*mustHaveExpression=*/ false,
          this.text
        );
  ...
  MessageFormatParser.prototype.ruleEnteredMustache =
    function ruleEnteredMustache() {
      this.parsedFn = null;
  ...
  MessageFormatParser.prototype.ruleAngularExpression =
    function ruleAngularExpression() {
  ...
  describe("$$ngMessageFormat", function () {
    describe("core", function () {
      ...
      beforeEach(function () {
        inject([
          ...
          "$interpolate",
          "$rootScope",
          function (messageFormat, parse, locale, interpolate, rootScope) {
      ...
      describe("mustache", function () {
        ...
        it("should return function that can be called with no context", inject(function (
          $interpolate
        ) {
        ...
        describe("plural", function () {
          ...
          it("with interpolation", function () {
            ...
            $rootScope.recipients.length = 2;
            expect(parsedFn($rootScope)).toEqual(
              "Harry Potter gave them a gift"
            );

            $rootScope.recipients.length = 1;
            expect(parsedFn($rootScope)).toEqual(
              "Harry Potter gave one gift to Alice"
            );
          ...
          it('with offset, interpolation, "#" symbol with and without escaping', function () {
            ...
            expect(parsedFn($rootScope)).toEqual(
              "Harry Potter gave Alice and 2 other people a gift (#=2)"
            );

            $rootScope.recipients.length = 2;
            expect(parsedFn($rootScope)).toEqual(
              "Harry Potter gave Alice and one other person a gift (#=1)"
            );

            $rootScope.recipients.length = 1;
            expect(parsedFn($rootScope)).toEqual(
              "Harry Potter gave one gift to Alice (#=0)"
            );

            $rootScope.recipients.length = 0;
            expect(parsedFn($rootScope)).toEqual(
              "Harry Potter gave no gifts (#=-1)"
            );
          });
        });
    ...
    describe("$interpolate", function () {
      ...
      describe("startSymbol", function () {
        it("should expose the startSymbol in run phase", inject(function (
          $interpolate
        ) {
      ...
      describe("endSymbol", function () {
        it("should expose the endSymbol in run phase", inject(function (
          $interpolate
        ) {
      ...
      it("should return the interpolation object when there are no bindings and textOnly is undefined", inject(function (
        $interpolate
      ) {
      ...
      it("should return undefined when there are no bindings and textOnly is set to true", inject(function (
        $interpolate
      ) {
        expect($interpolate("some text", true)).toBeUndefined();
      }));

      it("should return undefined when there are bindings and strict is set to true", inject(function (
        $interpolate
      ) {
        expect(
          $interpolate("test {{foo}}", false, null, true)({})
        ).toBeUndefined();
      }));
      ...
      it("should use custom toString when present", inject(function (
        $interpolate,
        $rootScope
      ) {
        var context = {
      ...
      it("should NOT use toString on array objects", inject(function (
        $interpolate
      ) {
        expect($interpolate("{{a}}")({ a: [] })).toEqual("[]");
      }));

      it("should NOT use toString on Date objects", inject(function (
        $interpolate
      ) {
      ...
      it("should return interpolation function", inject(function (
        $interpolate,
        $rootScope
      ) {
      ...
      it("should ignore undefined model", inject(function ($interpolate) {
        expect($interpolate("Hello {{'World'}}{{foo}}")({})).toBe(
          "Hello World"
        );
      }));

      it("should interpolate with undefined context", inject(function (
        $interpolate
      ) {
      ...
      describe("watching", function () {
        it("should be watchable with any input types", inject(function (
          $interpolate,
          $rootScope
        ) {
          var lastVal;
        ...
        it("should be watchable with literal values", inject(function (
          $interpolate,
          $rootScope
        ) {
          var lastVal;
        ...
        it("should respect one-time bindings for each individual expression", inject(function (
          $interpolate,
          $rootScope
        ) {
          var calls = [];
          $rootScope.$watch(
            $interpolate("{{::a | limitTo:1}} {{::s}} {{::i | number}}"),
            function (val) {
              calls.push(val);
            }
          );
        ...
        it("should stop watching strings with no expressions after first execution", inject(function (
          $interpolate,
          $rootScope
        ) {
          var spy = jasmine.createSpy();
        ...
        it("should stop watching strings with only constant expressions after first execution", inject(function (
          $interpolate,
          $rootScope
        ) {
          var spy = jasmine.createSpy();
      ...
      describe("interpolation escaping", function () {
        ...
        it("should support escaping interpolation signs", inject(function (
          $interpolate
        ) {
        ...
        it("should unescape multiple expressions", inject(function (
          $interpolate
        ) {
          expect(
            $interpolate("\\{\\{foo\\}\\}\\{\\{bar\\}\\} {{foo}}")(obj)
          ).toBe("{{foo}}{{bar}} Hello");
          expect(
            $interpolate("{{foo}}\\{\\{foo\\}\\}\\{\\{bar\\}\\}")(obj)
          ).toBe("Hello{{foo}}{{bar}}");
          expect(
            $interpolate("\\{\\{foo\\}\\}{{foo}}\\{\\{bar\\}\\}")(obj)
          ).toBe("{{foo}}Hello{{bar}}");
        ...
        it("should unescape incomplete escaped expressions", inject(function (
          $interpolate
        ) {
        ...
        it("should not unescape markers within expressions", inject(function (
          $interpolate
        ) {
          expect(
            $interpolate('{{"\\\\{\\\\{Hello, world!\\\\}\\\\}"}}')(obj)
          ).toBe("\\{\\{Hello, world!\\}\\}");
        ...
        it("should evaluate expressions between escaped start/end symbols", inject(function (
          $interpolate
        ) {
      ...
      describe("interpolating in a trusted context", function () {
        ...
        it("should NOT interpolate non-trusted expressions", inject(function (
          $interpolate,
          $rootScope
        ) {
          var scope = $rootScope.$new();
        ...
        it("should NOT interpolate mistyped expressions", inject(function (
          $interpolate,
          $rootScope
        ) {
          var scope = $rootScope.$new();
        ...
        it("should interpolate trusted expressions in a regular context", inject(function (
          $interpolate
        ) {
        ...
        it("should interpolate trusted expressions in a specific trustedContext", inject(function (
          $interpolate
        ) {
          var foo = sce.trustAsCss("foo");
          expect($interpolate("{{foo}}", true, sce.CSS)({ foo: foo })).toBe(
            "foo"
          );
        }));
        ...
        it("should NOT interpolate trusted expressions with multiple parts", inject(function (
          $interpolate
        ) {
          var foo = sce.trustAsCss("foo");
          ...
          expect(function () {
            ...
            "$interpolate",
            "noconcat",
            "Error while interpolating: {{foo}}{{bar}}\n" +
      ...
      describe("parseBindings", function () {
        it("should Parse Text With No Bindings", inject(function (
          $interpolate
        ) {
        ...
        it("should Parse Two Bindings With Text In Middle", inject(function (
          $interpolate
        ) {
      ...
      describe("isTrustedContext", function () {
        it("should NOT interpolate a multi-part expression when isTrustedContext is true", inject(function (
          $interpolate
        ) {
          ...
          expect(function () {
            ...
            "$interpolate",
            "noconcat",
            "Error while interpolating: constant/{{var}}\nStrict " +
          ...
          expect(function () {
            ...
            "$interpolate",
            "noconcat",
            "Error while interpolating: {{var}}/constant\nStrict " +
              "Contextual Escaping disallows interpolations that concatenate multiple expressions " +
              "when a trusted value is required.  See http://docs.angularjs.org/api/ng.$sce"
          );
          expect(function () {
        ...
        it("should interpolate a multi-part expression when isTrustedContext is false", inject(function (
          $interpolate
        ) {
          expect($interpolate("some/{{id}}")({})).toEqual("some/");
          expect($interpolate("some/{{id}}")({ id: 1 })).toEqual("some/1");
          expect($interpolate("{{foo}}{{bar}}")({ foo: 1, bar: 2 })).toEqual(
            "12"
          );
        }));
      });
