describe("", function () {
  ...
  var NgRepeatToggle = function (i) {
    var parent = element
      .all(by.repeater("(index, value) in [1,2,3]"))
      .get(i - 1);
    this.toggle = function () {
    ...
    this.isOpen = function () {
      return parent
        .element(by.binding("ngRepeatToggle.isOpen() | json"))
        .getText();
    };
    this.isOuterOpen = function () {
      return parent
        .element(by.binding("outerToggle.isOpen() | json"))
        .getText();
    };
  };
  var NgRepeatToggles = function () {
    var toggles = [1, 2, 3].map(function (i) {
      return new NgRepeatToggle(i);
    });
