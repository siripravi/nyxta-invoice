...
angular
  ...
  .controller("DocsController", [
    ...
    function (
      ...
      $scope.$on("$includeContentLoaded", function () {
        var pagePath = $scope.currentPage
        ...
        function docsPathWatchAction(path) {
          ...
          if (currentPage) {
            ...
            angular.forEach(pathParts, function (part) {
              ...
              breadcrumb.push({
                name:
                  (NG_PAGES[breadcrumbPath] && NG_PAGES[breadcrumbPath].name) ||
                  part,
                url: breadcrumbPath,
              });
              breadcrumbPath += "/";
   ...
   ***********************************/
      ...
      $scope.version =
        CURRENT_NG_VERSION.full + " " + CURRENT_NG_VERSION.codeName;
      $scope.loading = false;
      $scope.loadingError = false;
  ...
  .filter("errorLink", [
    ...
    function ($sanitize) {
      var LINKY_URL_REGEXP =
          /((ftp|https?):\/\/|(mailto:)?[A-Za-z0-9._%+-]+@)\S*[^\s.;,(){}<>]/g,
    ...
    function ($location, errorLinkFilter) {
      ...
      return {
        link: function (scope, element, attrs) {
          ...
          formattedText = encodeAngleBrackets(
            interpolate.apply(null, formatArgs)
          );
  ...
  .directive("plnkrOpener", [
    ...
    function ($q, getExampleData, formPostData, createCopyrightNotice) {
      return {
        ...
        controllerAs: "plnkr",
        template:
          '<button ng-click="plnkr.open($event)" class="btn pull-right"> <i class="glyphicon glyphicon-edit">&nbsp;</i> Edit in Plunker</button> ',
        controller: [
            ...
            ctrl.open = function (clickEvent) {
              ...
              ctrl.prepareExampleData().then(function () {
                angular.forEach(ctrl.example.files, function (file) {
                  postData["files[" + file.name + "]"] =
                    file.content + createCopyrightNotice(file.name);
                });
...
angular
  ...
  .controller("Error404SearchCtrl", [
    ...
    "docsSearch",
    function ($scope, $location, docsSearch) {
      docsSearch($location.path().split(/[/.:]/).pop()).then(function (
        results
      ) {
        $scope.results = {};
  ...
  .provider("docsSearch", function () {
    ...
    function localSearchFactory($http, $timeout, NG_PAGES) {
      ...
      var indexReadyPromise = $http
        .get("js/search-data.json")
        .then(function (response) {
    ...
    function ($document) {
      return function (scope, element, attrs) {
        ...
        angular.element($document[0].body).on("keydown", function (event) {
          var input = element[0];
          if (
            event.keyCode === FORWARD_SLASH_KEYCODE &&
            $document[0].activeElement !== input
          ) {
  ...
  .directive("versionPicker", function () {
    return {
      ...
      controller: [
        ...
        function VersionPickerController(
          $location,
          $window,
          CURRENT_NG_VERSION,
          ALL_NG_VERSIONS
          ...
          if (CURRENT_NG_VERSION.isSnapshot) {
            versionStr =
              CURRENT_NG_VERSION.distTag === "latest"
                ? "snapshot-stable"
                : "snapshot";
          }
