...
var Custom = {
  init: function () {
    ...
    for (aval = 0; aval < inputs.length; aval++) {
      if (
        (inputs[aval].type == "checkbox" || inputs[aval].type == "radio") &&
        inputs[aval].className == "styled"
      ) {
    ...
    for (aval = 0; aval < inputs.length; aval++) {
      if (inputs[aval].className == "styled") {
        ...
        for (b = 0; b < option.length; b++) {
          if (option[b].selected == true) {
            textnode = document.createTextNode(
              option[b].childNodes[0].nodeValue
            );
          }
        }
        span[aval] = document.createElement("span");
        ...
        if (!inputs[aval].getAttribute("disabled")) {
          ...
          inputs[aval].previousSibling.className = inputs[
            aval
          ].previousSibling.className += " disabled";
        }
      }
    }
  ...
  clear: function () {
    ...
    for (var b = 0; b < inputs.length; b++) {
      if (
        ...
        inputs[b].previousSibling.style.backgroundPosition =
