...
invApp.factory("getInvItems", function ($http, $q) {
  this.getlist = function (i, j) {
    ...
    return $http({
      method: "POST",
      url: "/statement/doc.items?id=" + i + "&invoice_id=" + j,
...
invApp.controller(
  "InvCtrl",
  function ($scope, $http, $window, $timeout, getInvItems) {
    $scope.yiiParams = {};
    ...
    $http
      .get(
        "/statement/doc.items?id=" +
          $scope.kf +
          "&invoice_id=" +
          $scope.header.invoice_id
      )
    ...
    $scope.insertRow = function (row_idx) {
      ...
      angular.forEach($scope.invoice.items, function (item, index) {
        ...
        if (index == row_idx) $scope.editingData[index + 1] = true;
        else $scope.editingData[index + 1] = false;
    ...
    $scope.deleteRow = function (index) {
      ...
      else $scope.invoice.items.splice(index, 1);
      $scope.subTotal();
    ...
    $scope.total = function () {
      ...
      return accounting.formatMoney(
        $scope.calculate_tax() + $scope.subTotal(),
        "$",
        2,
        ",",
        "."
    ...
    $scope.saveItems = function () {
      ...
      $http({
        ...
        url:
          ...
          $scope.header.invoice_id +
          "&customer_id=" +
          $scope.header.customer_id +
          "&venue_id=" +
          $scope.header.venue_id +
          "&event_date=" +
          $scope.header.event_date,
        data: note, //param method from jQuery
        headers: {
          "Content-Type": "application/x-www-form-urlencoded",
        }, //set the headers so angular passing info as form data (not request payload)
    ...
    $scope.modify = function (idx) {
      ...
      angular.forEach($scope.invoice.items, function (item, index) {
        //sum += item.quantity * item.price;
        if (index + 1 == idx) $scope.editingData[index + 1] = true;
        else $scope.editingData[index + 1] = false;
      });
