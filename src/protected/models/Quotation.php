...
class Quotation extends CActiveRecord
{
    ...
    public function rules()
    {
        ...
        return array(
            array('quotation_id', 'required'),
            //array('st_id, st_type', 'numerical', 'integerOnly'=>true),
            array('quotation_id', 'length', 'max' => 20),

            // The following rule is used by search().
            // @todo Please remove those attributes that should not be searched.
            //array('quotation_id,ship_date,venue_id,customer_no','required','on'=>'header'),
            array('st_id, st_type, quotation_id,from_date, to_date,customer_name, ship_name,ship_date', 'safe', 'on' => 'search'),
        );
    }
    ...
    /*public function search()
       {
           ...
           $criteria->compare('quotation_id',$this->quotation_id,true);
                   $criteria->compare('quotation.approval', 0);
    ...
    public static function moveItems($id, $inId)
    {
        $valid = false;
        $ret = false;
        $quot = Quotation::model()->findByPk($id); //echo $quot->st_id;
        $transaction =
