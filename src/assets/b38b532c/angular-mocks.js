        ...
        if (!$browser && timeout) {
          if (timeout.then) {
            timeout.then(function () {
              handlePrematureEnd(
                angular.isDefined(timeout.$$timeoutId) ? "timeout" : "abort"
              );
            });
          } else {
            $timeout(function () {
  ...
  function MockHttpExpectation(
    ...
    this.matchData = function (data) {
      ...
      if (expectedData && !angular.isString(expectedData)) {
        return angular.equals(
          angular.fromJson(angular.toJson(expectedData)),
    ...
    this.params = function (url) {
      ...
      return angular.extend(
        extractParamsFromQuery(queryStr),
        extractParamsFromPath(strippedUrl)
      );
    };
    ...
    function extractParamsFromPath(strippedUrl) {
      ...
      if (
        !expectedUrl ||
        !angular.isFunction(expectedUrl.test) ||
        !expectedKeys ||
        !expectedKeys.length
      )
        return keyObj;
    ...
    function extractParamsFromQuery(queryStr) {
      ...
      angular.forEach(keyValuePairs, function (pair) {
        ...
        if (angular.isDefined(key)) {
          var val = angular.isDefined(pair[1])
            ? tryDecodeURIComponent(pair[1])
            : true;
  ...
  angular.mock.$TimeoutDecorator = [
    ...
    function ($delegate, $browser) {
      ...
      $delegate.verifyNoPendingTasks = function () {
        ...
        if (pendingTasks.length) {
          var formattedTasks = $browser.defer
            .formatPendingTasks(pendingTasks)
            .join("\n  ");
      ...
      this.$get = [
        ...
        function ($controller, $injector, $rootScope) {
          return function $componentController(
            ...
            var candidateDirectives = directives.filter(function (
              directiveInfo
            ) {
