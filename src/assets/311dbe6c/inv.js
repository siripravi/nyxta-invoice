...
invApp.controller('InvCtrl',function ($scope,$http, $window,$timeout,getInvItems) {
  ...
  /* getInvItems.getlist($scope.kf,$scope.header.invoice_id).success(function(response)
    ...
    $http.get('/statement/doc.items?id=' +$scope.kf+"&invoice_id="+ $scope.header.invoice_id)
    ...
    $scope.saveItems = function(){
        ...
        $http({
            method: 'POST',
            url: '/statement/doc.items?id=' + $scope.kf+"&invoice_id="+$scope.header.invoice_id+"&customer_id="+$scope.header.customer_id+
